...
class Iw_vendeur(commands.Cog):
    ...
    async def iw_vendeur(self, ctx, status, user: discord.User, *, reason="Aucune raison n'a été renseignée !"):
        guild = ctx.guild
        if status == "accept":
            emuser = discord.Embed(title="Félicitations 🎉✨",
                                   description="Vous êtes **accepté dans l'équipe** des **Vendeurs du ScaryShop**. Vous pouvez commencer à découvrir vos accès sur le serveur Staff du ScaryShop. \n \n > ***Lien du serveur :***  https://discord.gg/S5dwdD97Cr \n \n Nous vous invitions à mentionner le responsable qui vous a fait passé votre entretien afin de prendre un rendez-vous pour les explications. \n \n **Bienvenue dans l'équipe !**",
            ...
            emuser = discord.Embed(title="Désolé 🥺",
                                   description=f"Malheureusement, votre entretien vendeur **ne donnera pas de suite**. Vous pourrez tout de même retenter votre chance dans 2 mois. \n \n > *Raison :* {reason} \n \n **Bon courage pour la suite !**",
                                   timestamp=datetime.datetime.utcnow(), color=0xFFA500)
            emuser.set_footer(icon_url=guild.icon_url, text=f"{guild.name}")
