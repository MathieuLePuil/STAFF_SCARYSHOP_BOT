...
class Rc_vendeur(commands.Cog):
    ...
    async def rc_vendeur(self, ctx, status, user: discord.User, *, reason="Aucune raison n'a été renseignée !"):
        guild = ctx.guild
        if status == "accept":
            emuser = discord.Embed(title="Félicitations 🎉✨",
                                   description="Votre candidature pour devenir vendeur sur le ScaryShop a été **retenue**. Celle-ci donne donc suite à un entretien vocal qui aura lieu sur le serveur discord réservé au Staff du ScaryShop. Si vous partagez le serveur à qui que ce soit, vous serez banni de nos services. \n \n > ***Lien du serveur :***  https://discord.gg/S5dwdD97Cr \n \n Lorsque vous arrivez dessus, veuillez indiquer dans #ma-demande la raison pour laquelle vous figurez sur le serveur. \n \n **Bon courage pour l'entretien !**",
            ...
            emuser = discord.Embed(title="Désolé 🥺",
                                   description=f"Malheureusement, votre candidature pour devenir vendeur sur le ScaryShop **n'a pas été retenue**. Vous pourrez tout de même retenter votre chance dans 2 mois. \n \n > *Raison :* {reason} \n \n **Bon courage pour la suite !**",
                                   timestamp=datetime.datetime.utcnow(), color=0xFFA500)
            emuser.set_footer(icon_url=guild.icon_url, text=f"{guild.name}")
