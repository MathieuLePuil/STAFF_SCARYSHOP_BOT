...
class Iw_moderateur(commands.Cog):
    ...
    @cog_ext.cog_slash(name = "iw_moderateur", description = "Accepte ou refuse un utilisateur dans l'équipe des moderateurs.")
    @commands.has_permissions(manage_roles = True)
    async def iw_moderateur(self, ctx, status, user : discord.User):
        guild = ctx.guild
        if status == "accept":
            emuser = discord.Embed(title = "Félicitations 🎉✨", description = "Vous êtes **accepté dans l'équipe** des **modérateurs du ScaryShop**. Vous pouvez commencer à découvrir vos accès sur le serveur Staff du ScaryShop. \n \n > ***Lien du serveur :***  https://discord.gg/S5dwdD97Cr \n \n Nous vous invitions à mentionner la personne qui vous a fait passé votre entretien afin de prendre un rendez-vous pour les explications. \n \n **Bienvenue dans l'équipe !**", timestamp=datetime.datetime.utcnow(), color = 0xFFA500)
            ...
            emuser = discord.Embed(title = "Désolé 🎉✨", description = "Malheureusement, votre entretien modérateur **ne donnera pas de suite**. Vous pourrez tout de même retenter votre chance dans 3 mois. \n \n **Bon courage pour la suite !**", timestamp=datetime.datetime.utcnow(), color = 0xFFA500)
            emuser.set_footer(icon_url=guild.icon_url, text=f"{guild.name}")
